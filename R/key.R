...
#' the `key` argument of any package function that requires it, but you do not
#' have to.
#'
...
#' call all the package API functions without having to provide the `key`
#' argument repeatedly.
#'
#' @section Key storage methods:
#' If you have already set your key globally somewhere using `eia_set_key()`,
#' `eia_get_key()` will retrieve it. You can add the `EIA_KEY = "yourkey"`
#' key-value pair to `options()` or as a system environment variable yourself
#' and `eia_get_key()` will pick it up as long as you use the name `EIA_KEY`.
#' For convenience you can do this in your R session with `eia_set_key()`. It
...
#' locations are checked in the order: package environment, `options()`, then
...
#' with `eia_get_key()`, just like those set in an R session with `eia_set_key()`
#' and `store = "sysenv"`. However, if you truly want the key to persist as an
#' environment variable when R terminates, you must manually add it somewhere
#' like `.Renviron`; `Sys.setenv` in R cannot achieve this.
#'
#' @param key character, API key.
#' @param store character, method for storing API key. See details.
#'
#' @return `eia_get_key()` returns the key string or `NULL` with a warning.
