...
(x <- eia_geoset(key, id[1], region[1], n = 3))
```
...
```{r geoset2}
(x <- eia_geoset(key, id, c("AK", "New England"), n = 1))
x$data[[1]]
```
...
Unfortunately, the relation endpoint does not seem to return any data from the  API even when using the examples in the API documentation from the EIA website.
