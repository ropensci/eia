...
no further action regarding the keyis required when using the package. See the vignette
on API details for more information about all the available options for key storage.

## EIA directory

Once the fully registered EIA API key is obtained and properly stored in theactive R session,
...
Similar to `facets`, `sort` requires a named list object with two components:
  ...
  * `"cols"` - a list of column names by which to order.
  * `"ordr"` - a single character value, either "asc" or "desc" for
