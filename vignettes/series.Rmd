...
x1 <- eia_series(key, id[1], n = 3)
x2 <- eia_series(key, id[2], n = 3)
x3 <- eia_series(key, id[3], n = 3)
```
...
*    Results are from the most recent; using only `n` will return data from the most recent time point.
*    You can use `start` and `end` instead of `n`
*    `n` is ignored if you use both `start` and `end`.
*    If you use only one of `start` or `end`, but do not provide `n`, you will receive everything forward or backward from that point, up to the maximum number of results allowed by the API.
*    Depending on the nature of the request, you may have to construct more than one call.


```{r series7, eval = FALSE}
eia_series(key, id, n = 10)
eia_series(key, id, end = 2016, n = 5)
eia_series(key, id, start = 2000, end = 2016)
```
...
Like `eia_seires`, these functions accept an `id` vector. They always return a tibble data frame.
